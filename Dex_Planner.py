...
class DexPlanner:
    def __init__(self, save_freq=0.5, max_history=20):
        ...
        # ROS订阅发布
        self.image_sub = rospy.Subscriber("/realsense_up/color/image_raw", Image, self.image_callback)
    ...
    def inference_new_target(self, obs):
        """推理新目标点（模拟推理过程）"""

        return [2.0, 1.0, 0.0, 0.0]  # [x, y, z, yaw]
    ...
    def visualize_data(local_traj, timestamp, history_obs=None):
        """
        ...
        # 保存图像
        save_path = os.path.join("scripts/result", f"traj_vis_{int(timestamp*1000)}.png")
    ...
    def run(self):
        ...
        rospy.info("[System] 路径规划器启动，等待初始目标点...")
        
        while not rospy.is_shutdown():
            
            with self.target_lock:
                target = self.current_target
                rospy.info("target:", target)
...
if __name__ == "__main__":
    planner = DexPlanner(save_freq=0.5, max_history=20)
    planner.run()
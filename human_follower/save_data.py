...
def save_move_obj_data_to_h5(observations, walk_path, h5_path, item):
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        for i, (pos, quat, yaw) in enumerate(walk_path):
            ...
            path_np[i, 7] = yaw
        f.create_dataset("rel_path", data=path_np, compression="gzip")
        f.create_dataset("object_environment", data=np.string_(item["object_environment"]))
        f.create_dataset("object_category", data=np.string_(item["object_category"]))
...
def process_one(src_file: Path, frames_root: Path, dst_root: Path, viz_root: Path|None, hist: int,
                sample_stride=1, future_dist=2.0):
    """
    ...
    with h5py.File(src_file, "r") as fin:
        ...
        frame_paths=[]
        try:
            frame_paths = fin["frames"][()]  # shape (N,)
        except Exception as e:
            print("[ERROR] Cannot read traj_1/observations/images:", e)
            return 0

        # decode bytes -> str
        frame_paths = [p.decode() if isinstance(p, bytes) else str(p) for p in frame_paths]
        ...
        with h5py.File(dst_h5, "w") as fout:
            ...
            # 遍历所有采样点
            for obs_idx in range(0, len(frame_paths), sample_stride):
...
if __name__=="__main__":
    ...
    args.src_dir = "data/raw_data/isaac"
    args.frames_dir ="data/frames/"
    args.dst_dir = "data/proc_data/isaac"

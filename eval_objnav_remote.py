...
def handle_client(conn, addr, robot):
    ...
    try:
        while True:
            ...
            raw_str = recv_all(conn, str_len)
            instruction = raw_str.decode('utf-8')
            robot.raw_lang = instruction
            # robot.instruction = instruction
            robot.instruction = f"Your task is: {robot.raw_lang} You are given a sequence of historical visual observations in temporal order (earliest first, latest last). Based on this sequence, predict your future movement trajectory."
            print(f"[Server] Received instruction: {robot.instruction}")
            ...
            actions, outputs = robot.eval_bc_raw()
            # actions[:, :2] = actions[:, :2] * 0.5
            #
            print(f"sub_reasions:{outputs}")
            if "Rotate" in outputs:
                actions[:, 0:2] = 0.0
                if actions[-1][-1] < 0.17:
                    print("reset")
                    actions[:, 2] = 0.5

...
def eval_bc(i,policy, target, deploy_env, policy_config, raw_lang=None, query_frequency=16, target_actions = None):
    ...
    with torch.inference_mode():
        ...
        result = plot_obs(i, all_actions,raw_lang, outputs, obs['top'][-1], human_position = target, target_actions=target_actions)
        return result
...
def find_all_hdf5(dataset_dir, skip_mirrored_data = False, rank0_print=print):
    dataset_paths = []
...
def filter_h5_by_objname(h5_paths, keep_obj_names):
    kept = []
    for p in h5_paths:
        try:
            with h5py.File(p, "r") as f:
                if "obj_name" not in f:
                    print(f"[WARN] No obj_name in {p}")
                    continue
                ...
                if obj_name not in keep_obj_names:
                    kept.append(p)
...
if __name__ == '__main__':
    ...
    query_frequency = 30
    samples = 20
    dataset_path = "/mnt/pfs/3zpd5q/code/zf/train_data/objnav_data"
    out_path = "/mnt/pfs/3zpd5q/code/zf/DexVLA/test/test_qwen2_dexvln_move_and_rotate_10w_60k/table"
    os.makedirs(out_path, exist_ok=True)
    # model_name = "/mnt/pfs/3zpd5q/code/zf/DexVLA/OUTPUT/qwen2_dexvln_move_and_rotate_10w_30k/checkpoint-30000" #ok
    model_name = "/mnt/pfs/3zpd5q/code/zf/DexVLA/OUTPUT/qwen2_dexvln_move_and_rotate_10w_60k_4/checkpoint-60000"
    keep_obj_names = ["table"]
    # keep_obj_names = [""]
    ...
    raw_paths = find_all_hdf5(dataset_path)
    dataset_path_list_list = filter_h5_by_objname(raw_paths, keep_obj_names)
    sampled_episodes = random.sample(dataset_path_list_list, samples)
    ...
    for ep_file in sampled_episodes:
        with h5py.File(ep_file, 'r') as f:
            ...
            instruction = f"Move next to a {obj_name} in the current environment by rotating or moving"
            # instruction = f['language_raw'][()].decode('utf-8')
            ...
            # instruction = f['obj_name'][()].decode('utf-8')
            # instruction = "bed"
            # raw_lang = f"Navigate toward the {instruction} based on the visual observations and predict the future movement trajectory."
            result_image = eval_bc(
                step_idx, policy, None,
                agilex_bot, policy_config,
                raw_lang=raw_lang,
            ...
            # 保存结果
            out_file = Path(out_path) / Path(selected_frames[-1]).name
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            cv2.imwrite(out_file, cv2.cvtColor(result_image, cv2.COLOR_RGB2BGR))
